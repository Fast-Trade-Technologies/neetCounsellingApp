...
class CoursesApi {
  ...
  /// GET {{base_url}}/courses. Params in query (e.g. course_id, roles, degree_type_id).
  /// nLoginUserIdNo sent in header. Returns (true, dataMap, null) on success.
  static Future<(bool success, Map<String, dynamic>? data, String? errorMessage)> getCourses({
    Map<String, dynamic>? queryParameters,
    bool showLoader = true,
  }) async {
    final userId = AppStorage.userId;
    if (userId == null || userId.isEmpty) {
      return (false, null, 'Please sign in to load courses');
    }
    try {
      ...
      if (response.statusCode != 200) {
        ...
        return (false, null, msg ?? 'Failed to load courses');
      }
      ...
      return (false, null, message ?? 'Failed to load courses');
