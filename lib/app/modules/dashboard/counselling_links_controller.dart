...
class CounsellingLinksController extends GetxController {
  ...
  final RxList<CounsellingLinkItem> list = <CounsellingLinkItem>[].obs;
  ...
  Future<void> loadData() async {
    ...
    final (success, data, msg) = await DashboardApi.getDashboard(
      showLoader: false,
      counsellingStateId: selectedStateId.value.isEmpty ? null : selectedStateId.value,
    );
    loading.value = false;
    if (success && data != null && data.counsellingLinks != null) {
      list.assignAll(data.counsellingLinks!);
    } else {
      error.value = msg ?? 'Failed to load counselling links';
    }
  }
}
